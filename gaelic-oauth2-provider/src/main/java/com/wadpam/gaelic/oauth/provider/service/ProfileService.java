...
public class ProfileService extends MardaoCrudService<Do2pProfile, Long, Do2pProfileDao> {
    ...
    public String create(Do2pProfile domain) {
        ...
        // super.create(domain);
        return createProfile(domain);
    }

    /**
     * There is some filter which need to be applied for other fields. If the exended class call super.create, it will always
     * checks the user name only
     * 
     * @param domain
     */
    protected String createProfile(Do2pProfile domain) {
        // make sure ID is generated:
        super.create(domain);
